...
def venta(request):
    cantidad = request.GET.get('cantidad', '')
    articulo = Articulo.objects.get(codigo_articulo= request.GET.get('codigo', ''))
    if request.session['ventaId'] == '':
        dateNow = datetime.now()
        print(dateNow)
        if cantidad:
        ...
        if cantidad:
            if int(cantidad) <= articulo.stock:
                ...
                subTotal = float(int(cantidad) * articulo.precio_unidad)
                form2 = DetalleForm(
                    {'cantidad': cantidad, 'precio': articulo.precio_unidad, 'sub_total': subTotal, 'id_articulo': articulo.pk,
                     'id_venta': ventaId})
                if form2.is_valid():
                    form2.save()
                    # Editar el registro ya existe de articulo para restar el stock
